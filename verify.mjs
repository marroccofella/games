...
const audioExtension = /\.(?:aac|flac|m4a|mp3|ogg|opus|wav|webm)$/i;
...
async function entriesFor(directory) {
  if (!directoryEntries.has(directory)) directoryEntries.set(directory, await readdir(directory));
...
async function resolveLocalReference(page, { kind, value }) {
  ...
  if (reference.startsWith("/")) {
    if (!(kind === "href" && reference === "/")) failures.push(`${path.relative(root, page)} uses unsafe root-absolute ${reference}`);
    return;
  }
...
const productFiles = allFiles.filter((file) => file === productRoot || file.startsWith(`${productRoot}${path.sep}`));
...
for (const key of ["ArrowLeft", "ArrowRight", "KeyA", "KeyD", "KeyE", "KeyM", "KeyO", "KeyP", "KeyR", "KeyV", "Space"]) {
  assert.ok([...scriptText.values()].some((source) => source.includes(key)), `cabinet must ship ${key}`);
}
assert.ok([...scriptText.values()].some((source) => /WITHHELD[^\n]{0,80}LISTENING VERDICT/i.test(source)), "cabinet must visibly disclose that the voice candidate is withheld");
...
for (const file of textualProductFiles) {
  ...
  const auditableSource = file.endsWith(".js")
    ...
    : source;
  assert.doesNotMatch(auditableSource, /\.(?:aac|flac|m4a|mp3|ogg|opus|wav|webm)(?:[?#"')\s]|$)/i, `${path.relative(productRoot, file)} must not reference unaccepted audio`);
}

const entryName = scripts.find((name) => /^index-[\w-]+\.js$/.test(name));
assert.ok(entryName, "cabinet must contain one Vite entry chunk");
...
await assert.rejects(stat(path.join(root, "willy")), "obsolete staging folder must not ship");
