...
function setControl(name, active) {
	...
	if (name === "jump") {
		if (active && !controls.jumpHeld) controls.jumpQueued = true;
		controls.jumpHeld = active;
		return;
	}
...
var DISTRICT_SONICS = Object.freeze([
	Object.freeze({
		id: "metalife-lobby",
		root: 84,
		wave2d: "square",
	...
	Object.freeze({
		...
		wave3d: "sawtooth",
		filter: 2480,
		label: "CONTROLLED OVERLOAD"
	})
]);
var RECEIPT_PITCHES = Object.freeze([
	330,
	392,
	440,
	523.25,
...
function receiptPitch(receiptCount) {
	return RECEIPT_PITCHES[(Number.isFinite(receiptCount) ? Math.max(0, Math.floor(receiptCount)) : 0) % RECEIPT_PITCHES.length];
}
function musicFrequency(districtIndex, patternStep) {
	const district = DISTRICT_SONICS[Math.max(0, Math.min(DISTRICT_SONICS.length - 1, districtIndex))];
	if (patternStep == null) return null;
	return district.root * 2 ** (patternStep / 12);
}
//#endregion
//#region app/game/audio.ts
var context = null;
var master = null;
var musicBus = null;
var sfxBus = null;
...
function ensureAudioGraph() {
	...
	sfxBus.gain.value = .38;
	uiBus.gain.value = .3;
	voiceBus.gain.value = .82;
	musicBus.connect(master);
	sfxBus.connect(master);
	uiBus.connect(master);
	voiceBus.connect(master);
	master.connect(limiter).connect(context.destination);
	introVoice ??= createIntroVoice();
	if (introVoice) {
		voiceSource = context.createMediaElementSource(introVoice);
		voiceSource.connect(voiceBus);
	}
}
...
function releaseOldestOscillator() {
	const oldest = activeOscillators.values().next().value;
	if (!oldest) return;
	activeOscillators.delete(oldest);
	try {
...
function spawnTone({ frequency, duration, wave = "square", gain = .12, at, destination, endFrequency, filterFrequency }) {
	...
	if (endFrequency && endFrequency > 0) oscillator.frequency.exponentialRampToValueAtTime(endFrequency, end);
...
function spectralAsterisk(spokes, at = context?.currentTime ?? 0) {
	const partials = [
		168,
		420,
		672,
	...
	for (let index = 0; index < Math.min(5, Math.max(1, spokes)); index += 1) spawnTone({
		frequency: partials[index],
		endFrequency: partials[index] * 1.04,
		duration: .11,
...
function playSfx(name, detail = {}) {
	...
	switch (name) {
		...
		case "receipt":
			spawnTone({
				frequency: receiptPitch(detail.receiptCount ?? 0),
				endFrequency: receiptPitch(detail.receiptCount ?? 0) * 1.12,
				duration: .15,
				wave: "triangle",
				gain: .105,
				destination: sfxBus
...
function scheduleMusicStep() {
	...
	if (musicStep % 6 === 0) appliedMusic = desiredMusic;
	const patternStep = MUSIC_PATTERN[musicStep % MUSIC_PATTERN.length];
	const district = DISTRICT_SONICS[appliedMusic.districtIndex] ?? DISTRICT_SONICS[0];
	const frequency = musicFrequency(appliedMusic.districtIndex, patternStep);
	if (frequency) {
		spawnTone({
			frequency,
			duration: MUSIC_STEP_SECONDS * .78,
			wave: appliedMusic.renderMode === "2d" ? district.wave2d : district.wave3d,
			gain: appliedMusic.renderMode === "2d" ? .035 : .045,
			at: nextMusicTime,
			destination: musicBus,
			filterFrequency: district.filter
		});
		if (appliedMusic.renderMode === "3d" && musicStep % 3 === 0) spawnTone({
			...
			at: nextMusicTime,
			destination: musicBus,
			filterFrequency: district.filter * .72
		});
	}
...
function pumpMusic() {
	if (!context || !desiredMusic.active) return;
	if (nextMusicTime < context.currentTime) nextMusicTime = context.currentTime + .04;
	while (nextMusicTime < context.currentTime + .22) scheduleMusicStep();
}
function startMusicScheduler() {
	if (!context || musicTimer !== null || !desiredMusic.active || muted) return;
	nextMusicTime = context.currentTime + .04;
	pumpMusic();
	musicTimer = window.setInterval(pumpMusic, 80);
}
...
function setMusicState(state) {
	desiredMusic = {
		active: state.active,
		districtIndex: Math.max(0, Math.min(DISTRICT_SONICS.length - 1, state.districtIndex)),
...
function stopIntroVoice() {
	...
	try {
		introVoice.pause();
		if (introVoice.readyState > 0) introVoice.currentTime = 0;
	} catch {}
...
function stopAudio() {
	...
	stopIntroVoice();
	for (const oscillator of activeOscillators) try {
		oscillator.stop();
	} catch {}
...
function readRenderMode(getStorage) {
	try {
		const value = (getStorage?.())?.getItem("freeloader42-mode");
		return value === "2d" || value === "3d" ? value : null;
	} catch {
		return null;
	}
}
function writeRenderMode(getStorage, mode) {
	try {
		(getStorage?.())?.setItem("freeloader42-mode", mode);
		return true;
	} catch {
		return false;
	}
...
//#endregion
//#region app/game/store.ts
var roomIds = new Set(ROOMS.map((room) => room.id));
...
var freshRun = () => ({
	...
	deaths: 0,
	deathSerial: 0,
	playerX: ROOMS[0].start.x,
	wildcard: "ready",
	notice: ROOMS[0].intro
});
var useGameStore = create((set, get) => ({
	...
	setRenderMode: (renderMode) => {
		set({ renderMode });
		if (typeof window !== "undefined") writeRenderMode(() => window.localStorage, renderMode);
	},
	hydrateBest: () => {
		if (typeof window === "undefined") return;
	...
	activateWildcard: () => {
		const state = get();
		if (state.phase !== "playing" || state.wildcard !== "ready") return false;
		set({
			wildcard: "active",
			notice: "* WILDCARD ACTIVE // ONE LOCAL RULE REWRITTEN FOR 4.2 SECONDS"
		});
		return true;
	},
	expireWildcard: () => set((state) => state.wildcard === "active" ? {
		wildcard: "spent",
...
var SceneBoundary = class extends import_react.Component {
	...
	componentDidCatch(error, info) {
		console.error("Playfield failed to initialise", error, info.componentStack);
	}
	render() {
		if (this.state.failed) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "scene-error",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "PLAYFIELD DEGRADED" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "The scene chunk declined further work. The briefing remains available; reload to negotiate again." })]
		});
...
function FreeloaderGame() {
	...
	const banked = useGameStore((state) => state.banked);
	...
	(0, import_react.useEffect)(() => {
		setMusicState({
			...
			districtIndex: Math.floor(roomIndex / 6),
			renderMode
		});
	}, [
		phase,
	...
	(0, import_react.useEffect)(() => {
		...
		const focusable = () => Array.from(modalCardRef.current?.querySelectorAll("button:not([disabled]), [href], [tabindex]:not([tabindex=\"-1\"])") ?? []);
		const trapFocus = (event) => {
			if (modalCardRef.current && event.target instanceof Node && !modalCardRef.current.contains(event.target)) focusable()[0]?.focus();
		};
		const trapKeys = (event) => {
			if (event.key === "Tab") {
				const controls = focusable();
				if (controls.length === 0) return;
				const active = document.activeElement instanceof HTMLElement ? controls.indexOf(document.activeElement) : -1;
	...
	(0, import_react.useEffect)(() => {
		const keyDown = (event) => {
			...
			if (!event.repeat && event.code === "KeyR" && game.phase !== "menu" && confirmPortfolioRestart()) {
				unlockAudio();
				stopIntroVoice();
				clearControls();
				game.restart();
			}
			if (!event.repeat && event.code === "KeyM" && game.phase !== "menu") {
				unlockAudio();
				game.toggleMute();
			}
	...
	const begin = () => {
		if (hasProgress && !window.confirm("Discard the saved portfolio and begin again at Property 01?")) return;
		unlockFreshRunAudio();
		clearControls();
	...
	const restart = () => {
		if (!confirmPortfolioRestart()) return;
		unlockAudio();
		stopIntroVoice();
		clearControls();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				inert: modalOpen ? true : void 0,
				"aria-hidden": modalOpen || void 0,
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SceneBoundary, { children: renderMode === "3d" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_react.Suspense, {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: modalOpen,
							className: autopilot ? "mode-active" : "",
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							...
							disabled: modalOpen,
							onClick: swapMode,
							"aria-label": renderMode === "3d" ? "Switch to 2D mode" : "Switch to 3D mode",
							children: renderMode === "3d" ? "2D" : "3D"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: modalOpen,
			...
			phase === "menu" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				className: "boot-card",
				"aria-labelledby": "game-title",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "kicker",
						children: "THE FREELOADER'S GUIDE // PORTFOLIO 0X2A"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h1", {
						...
						children: [
							...
							"FORTY-TWO",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: "PROPERTIES." })
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "briefing",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "mode-row",
						role: "group",
						"aria-label": "Choose renderer",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							"aria-pressed": renderMode === "2d",
							className: renderMode === "2d" ? "mode-active" : "",
							onClick: () => useGameStore.getState().setRenderMode("2d"),
							children: "2D CRT"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							"aria-pressed": renderMode === "3d",
							className: renderMode === "3d" ? "mode-active" : "",
							onClick: () => useGameStore.getState().setRenderMode("3d"),
							children: "3D FIELD"
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [hasProgress && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							...
							type: "button",
							onClick: continueRun,
							children: "[ CONTINUE SAVED PORTFOLIO ]"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: hasProgress ? "secondary-action" : "primary-action",
							type: "button",
							onClick: begin,
							children: hasProgress ? "[ DISCARD CHECKPOINT + START AGAIN ]" : "[ ENTER PROPERTY 01 ]"
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "controls-line",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "A / D" }),
							" LEG IT ",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						...
						children: "An independent 42.uk room-platformer set in The Freeloader's Guide universe."
					})
				]
			}),
			phase !== "menu" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "hud-metric latency-metric",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "LATENCY TAX" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: Math.ceil(latency).toString().padStart(3, "0") }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "latency-track",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { style: { width: `${Math.max(0, Math.min(100, latency / 100 * 100))}%` } })
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "hud-metric",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "PROPERTY" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: [
							String(roomIndex + 1).padStart(2, "0"),
							"/",
							String(TOTAL_ROOMS).padStart(2, "0")
						] })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "PORTFOLIO" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: [
							banked,
							"/",
							TOTAL_SHARDS
						] })]
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "hud-metric optional-metric",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "CORE DUMPS" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: deaths.toString().padStart(2, "0") })]
					})
				]
			}),
			...
			phase === "won" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "kicker",
						children: ["EVIDENCE BUNDLE // RUN ", String(runSerial).padStart(4, "0")]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
						id: "win-title",
						children: [
							"REET.",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: "LARGELY YOUR PROPERTY NOW." })
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "All forty-two properties certified. T'portfolio has handed remaining bother to a newer, less informed version of you. Net positive, statistically." }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "result-grid",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "RECEIPTS" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: [
								banked,
								"/",
								TOTAL_SHARDS
							] })] }),
			...
			phase === "playing" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TouchButton, {
					control: "left",
					label: "Move left"
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				...
				role: "status",
				"aria-live": "polite",
				children: phase === "menu" ? "" : notice
			})
		]
	});
}
//#endregion
//#region arcade/main.tsx
